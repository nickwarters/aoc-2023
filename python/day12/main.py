...
def count_variations(springs: str, broken_springs: List[int], i: int, bi: int, c: int):
    if i == len(springs):
    ...
    for ch in ('.', '#'):
        if springs[i] == ch or springs[i] == '?':
            if ch == '.' and c == 0:
                ...
                count += count_variations(springs, broken_springs, i + 1, bi, c)
            elif ch == '.' and bi < len(broken_springs) and c > 0 and broken_springs[bi] == c:
                # we have reached the end of a set of possibly broken and known broken springs. Start checking for the next group of springs and reset the count
                count += count_variations(springs, broken_springs, i + 1, bi + 1, 0)
            elif ch == '#':
                # either the spring is known broken, or possibly broken, using the current broken index we are checking for, increment the broken count
                count += count_variations(springs, broken_springs, i + 1, bi, c + 1)
